...
pub async fn run_server(file_path: PathBuf, config: RuntimeConfig) -> anyhow::Result<()> {
    ...
    let debounce_ms = config.file_config.preview.debounce_ms;
    tokio::spawn(async move {
        if let Err(e) = watcher::watch_file(watch_file, watch_state, debounce_ms).await {
...
async fn index_handler(State(state): State<Arc<AppState>>) -> Html<String> {
    ...
    let html = markdown::render_for_preview(&content, &state.theme, &state.file_path);
    Html(html)
}
