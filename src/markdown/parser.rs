...
pub fn render_html(input: &str, render_config: &RenderConfig) -> String {
    ...
    let callout_map = if render_config.callout {
        callout::detect_callouts(input, &events_with_ranges)
    } else {
        HashMap::new()
    };
    ...
    while i < len {
        ...
        match event {
            Event::Start(tag) => {
                let tag_html =
                    start_tag_html(tag, line, &callout_map, range.start, input, in_table_head);
            ...
            Event::Text(text) => {
                html.push_str(&html_escape(&render_text(text, render_config)));
            }
            Event::Code(code) => {
...
fn render_text(text: &str, render_config: &RenderConfig) -> String {
    if render_config.emoji {
...
fn replace_emoji_shortcodes(text: &str) -> String {
    const EMOJI_REPLACEMENTS: [(&str, &str); 8] = [
        (":smile:", "😄"),
        (":rocket:", "🚀"),
        (":warning:", "⚠️"),
        (":bulb:", "💡"),
    ...
    for (shortcode, emoji) in EMOJI_REPLACEMENTS {
        rendered = rendered.replace(shortcode, emoji);
    }
    rendered
}
...
mod tests {
    ...
    #[test]
    fn renders_image_with_closed_alt_attribute() {
        let html = render_html("![Alt text](image.png \"Title\")\n", &RenderConfig::default());
        assert!(html.contains("<img src=\"image.png\" title=\"Title\" alt=\"Alt text\" /></span>"));
    }

    #[test]
    fn renders_table_header_cells_as_th() {
        let html = render_html("| Name | Value |\n| --- | --- |\n| A | B |\n", &RenderConfig::default());
    ...
    #[test]
    fn disables_callout_rendering_when_configured() {
    ...
    #[test]
    fn renders_emoji_shortcodes_when_enabled() {
        let html = render_html("Ship it :rocket:\n", &RenderConfig::default());
        assert!(html.contains("🚀"));
    }

    #[test]
    fn preserves_emoji_shortcodes_when_disabled() {
        let cfg = RenderConfig {
            emoji: false,
            ..RenderConfig::default()
        };
