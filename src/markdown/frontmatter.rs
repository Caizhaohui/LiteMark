    ...
    pub fn to_html(&self) -> String {
        ...
        for (key, value) in &self.extra {
            let rendered = match value {
                ...
                serde_yaml::Value::String(v) => html_escape(v),
                other => html_escape(&serde_yaml::to_string(other).unwrap_or_default().replace('\n', " ").trim().to_string()),
            };
            if !rendered.is_empty() {
                html.push_str(&format!(
                    "  <div class=\"fm-row\"><span class=\"fm-key\">{}</span><span class=\"fm-val\">{}</span></div>\n",
                    html_escape(key),
                    rendered
                ));
            }
